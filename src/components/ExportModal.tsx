  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl h-[600px] flex flex-col">
        ...
        <Tabs value={activeFormat} onValueChange={(value) => setActiveFormat(value as ExportFormat)} className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex-1 overflow-auto mt-4 bg-muted rounded-lg overflow-x-auto">
            <TabsContent value="css" className="mt-0 h-full">
              <pre className="p-4 text-sm font-mono">{getExportData()}</pre>
            </TabsContent>
          ...
          <div className="mt-4 flex items-center justify-between gap-4 pt-4 border-t min-h-[48px]">
            <div className="flex items-center gap-4">
              {activeFormat === "text" && (
                <>
                  <div className="flex items-center space-x-2">
                    <Checkbox id="numbered" checked={plainTextNumbered} onCheckedChange={(checked) => setPlainTextNumbered(checked as boolean)} />
                    <Label htmlFor="numbered" className="cursor-pointer">
                      {t("numbered")}
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Checkbox id="includeHash" checked={plainTextIncludeHash} onCheckedChange={(checked) => setPlainTextIncludeHash(checked as boolean)} />
